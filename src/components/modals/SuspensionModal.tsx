...
export default function SuspensionModal({ visible, onClose, onSaved }: Props) {
  ...
  async function handleSave() {
    ...
    const fechaIso = fechaIncidente.trim().length === 16 ? `${fechaIncidente.trim()}:00` : fechaIncidente.trim();
    ...
    try {
      ...
      const msg = e?.response?.data?.message || e?.message || 'Error al registrar sanción';
        ...
        <View style={styles.modal}>
          ...
          <ScrollView style={styles.form} showsVerticalScrollIndicator={false} keyboardShouldPersistTaps="handled">
            ...
            <View style={styles.pillContainer}>
              <TouchableOpacity
                style={[styles.pill, tipoSuspencion === 2 && styles.pillDanger]}
                onPress={() => setTipoSuspencion(2)}
              >
                <Text style={[styles.pillText, tipoSuspencion === 2 && styles.pillTextActive]}>
                  Suspensión
                </Text>
              </TouchableOpacity>
              <TouchableOpacity
                style={[styles.pill, tipoSuspencion === 1 && styles.pillWarning]}
                onPress={() => setTipoSuspencion(1)}
              >
                <Text style={[styles.pillText, tipoSuspencion === 1 && styles.pillTextActive]}>
                  Llamado de atención
                </Text>
              </TouchableOpacity>
            </View>
