...
export default function SuspensionesListScreen() {
        ...
        { text: 'Cancelar', style: 'cancel' },
        {
          ...
          onPress: async () => {
            try {
              ...
              const msg = e?.response?.data?.message || e?.message || 'Error al eliminar sanción';
  ...
  function renderItem({ item }: { item: GetSuspencionDTO }) {
    const fechaIncidente = new Date(item.fechaIncidente).toLocaleDateString('es-AR');
    const fechaFin = new Date(item.fechaFin).toLocaleDateString('es-AR');
    const esSuspension = item.tipoSuspencion === 2;
    const nombreArbitro = `${item.arbitro?.apellido || ''}, ${item.arbitro?.nombre || ''}`.trim() || 'Árbitro';
    ...
    return (
      <View style={[styles.card, esSuspension && styles.cardSuspension]}>
        <View style={styles.cardHeader}>
          ...
          <View style={[styles.badge, { backgroundColor: esSuspension ? '#e74c3c' : '#f39c12' }]}>
            <Text style={styles.badgeText}>{TIPO_LABEL[item.tipoSuspencion] || 'Sanción'}</Text>
          </View>
        </View>
  ...
  return (
    <View style={styles.container}>
      <View style={styles.topBar}>
        <Text style={styles.title}>Sanciones ({suspensiones.length})</Text>
        {canManage && (
      ...
      <FlatList
        ...
        ListEmptyComponent={<Text style={styles.empty}>Sin sanciones o suspensiones</Text>}
...
const styles = StyleSheet.create({
  ...
  topBar: {
    ...
    paddingHorizontal: scaleFont(16), paddingBottom: scaleFont(10), paddingTop: scaleFont(44),
    backgroundColor: '#fff', borderBottomWidth: 1, borderBottomColor: '#e2e8f0',
  },
  title: { fontSize: scaleFont(20), fontWeight: 'bold', color: '#0f172a' },
  btnAdd: {
    backgroundColor: '#c0392b', paddingVertical: scaleFont(8), paddingHorizontal: scaleFont(14),
  ...
  arbitro: { fontSize: scaleFont(15), fontWeight: '700', color: '#0f172a', flex: 1 },
  badge: { borderRadius: 6, paddingHorizontal: scaleFont(8), paddingVertical: 3 },
