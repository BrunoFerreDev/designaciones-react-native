...
import {
  ...
  RefreshControl, Switch, TouchableOpacity, TextInput, Alert,
} from 'react-native';
...
export default function ArbitrosListScreen() {
  ...
  async function toggleDisponibilidad(
    ...
    try {
      ...
      const dom = campo === 'disponibleDomingo' ? valor : item.disponibleDomingo;
      ...
      setArbitros((prev) =>
        prev.map((a) => (a.idArbitro === item.idArbitro ? { ...a, [campo]: valor } : a))
      );
    } catch (e: any) {
      const msg = e?.response?.data?.message || e?.message || 'Error al actualizar disponibilidad';
  ...
  async function toggleEstadoSistema(item: GetArbitroDTO) {
    if (!canManage) {
      Alert.alert('Permiso denegado', 'Solo administradores o secretarios pueden activar/desactivar árbitros');
      return;
    }
    ...
    try {
      ...
      setArbitros((prev) =>
        prev.map((a) => (a.idArbitro === item.idArbitro ? { ...a, estadoSistema: nuevoEstado } : a))
      );
    } catch {
      ...
      try {
        ...
        const msg = err?.response?.data?.message || err?.message || 'Error al cambiar estado del árbitro';
        Alert.alert('Error', msg);
      }
    }
  }
  ...
  function renderItem({ item }: { item: GetArbitroDTO }) {
    const canEditDisp = isDesignador || item.idArbitro === authArbitro?.idArbitro;
            ...
            <View style={styles.nameRow}>
              ...
              {activeTab === 'todos' && (
                <View
                  style={[
                    styles.statusBadge,
                    item.estadoSistema ? styles.statusBadgeActive : styles.statusBadgeInactive,
                  ]}
                >
                  <Text
                    style={[
                      styles.statusBadgeText,
                      item.estadoSistema ? styles.statusTextActive : styles.statusTextInactive,
                    ]}
                  >
          ...
          <View style={styles.disponibilidad}>
            <View style={styles.dispItem}>
              ...
              <Switch
                ...
                onValueChange={(v) => {
                  if (canEditDisp) toggleDisponibilidad(item, 'disponibleSabado', v);
                }}
            ...
            <View style={styles.dispItem}>
              ...
              <Switch
                ...
                onValueChange={(v) => {
                  if (canEditDisp) toggleDisponibilidad(item, 'disponibleDomingo', v);
                }}
                disabled={!canEditDisp}
                trackColor={{ false: '#cbd5e1', true: '#27ae60' }}
              />
          ...
          <View style={styles.systemToggleRow}>
            <View style={{ flex: 1 }}>
              <Text style={styles.systemToggleTitle}>
                {item.estadoSistema ? 'Habilitado en el Sistema' : 'Inhabilitado / De baja'}
              </Text>
              <Text style={styles.systemToggleSub}>
                {item.estadoSistema ? 'Puede recibir designaciones activas' : 'No aparece en selección de designaciones'}
              </Text>
            </View>
            <Switch
  ...
  if (loading) return <ActivityIndicator style={{ flex: 1 }} size="large" color="#1a1a2e" />;
  ...
  return (
    <View style={styles.container}>
      ...
      <View style={styles.tabsContainer}>
        <TouchableOpacity
          style={[styles.tabButton, activeTab === 'activos' && styles.tabButtonActive]}
      ...
      <TextInput
        style={styles.search}
        placeholder={activeTab === 'activos' ? "Buscar en árbitros activos..." : "Buscar en todos los árbitros..."}
      ...
      <FlatList
        ...
        contentContainerStyle={{ paddingHorizontal: scaleFont(16), paddingBottom: scaleFont(80) }}
      />
...
const styles = StyleSheet.create({
  ...
  topBar: {
    ...
    paddingHorizontal: scaleFont(16), paddingBottom: scaleFont(8), paddingTop: scaleFont(44),
    backgroundColor: '#fff',
  },
  title: { fontSize: scaleFont(20), fontWeight: 'bold', color: '#0f172a' },
  subtitle: { fontSize: scaleFont(12), color: '#64748b', marginTop: 2 },
  btnAdd: {
    backgroundColor: '#1a1a2e', paddingVertical: scaleFont(8), paddingHorizontal: scaleFont(14),
    borderRadius: 8, elevation: 2,
  },
  btnAddText: { color: '#fff', fontSize: scaleFont(12), fontWeight: '700' },
  tabsContainer: {
    flexDirection: 'row', backgroundColor: '#fff',
    paddingHorizontal: scaleFont(16), paddingBottom: scaleFont(10),
    borderBottomWidth: 1, borderBottomColor: '#e2e8f0', gap: 8,
  },
  tabButton: {
    flex: 1, paddingVertical: scaleFont(8), alignItems: 'center',
  ...
  search: {
    marginHorizontal: scaleFont(16), marginVertical: scaleFont(10), borderWidth: 1, borderColor: '#cbd5e1',
    borderRadius: 10, padding: scaleFont(10), fontSize: scaleFont(14), backgroundColor: '#fff', color: '#0f172a',
  },
  card: {
    backgroundColor: '#fff', borderRadius: 12, padding: scaleFont(14),
    marginBottom: scaleFont(10), elevation: 1, borderWidth: 1, borderColor: '#e2e8f0',
  },
  cardHeader: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 6 },
  nameRow: { flexDirection: 'row', alignItems: 'center', gap: 8, flexWrap: 'wrap' },
  nombre: { fontSize: scaleFont(15), fontWeight: '700', color: '#0f172a' },
  statusBadge: { borderRadius: 4, paddingHorizontal: scaleFont(6), paddingVertical: 2 },
  ...
  rolesRow: { flexDirection: 'row', flexWrap: 'wrap', gap: 6, marginVertical: 6 },
  roleBadge: {
    backgroundColor: '#e2e8f0', borderRadius: 4,
    paddingHorizontal: scaleFont(6), paddingVertical: 2,
  },
  roleText: { fontSize: scaleFont(10), fontWeight: '700', color: '#334155' },
  disponibilidad: {
    flexDirection: 'row', gap: scaleFont(16), marginTop: 8,
    paddingTop: scaleFont(8), borderTopWidth: 1, borderTopColor: '#f1f5f9',
  },
  dispItem: { flexDirection: 'row', alignItems: 'center', gap: 6 },
  dispLabel: { fontSize: scaleFont(12), color: '#475569', fontWeight: '600' },
  systemToggleRow: {
    flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center',
    marginTop: 8, paddingTop: scaleFont(8), borderTopWidth: 1, borderTopColor: '#f1f5f9',
  },
  systemToggleTitle: { fontSize: scaleFont(12), fontWeight: '700', color: '#0f172a' },
  systemToggleSub: { fontSize: scaleFont(10), color: '#64748b', marginTop: 1 },
  empty: { textAlign: 'center', color: '#94a3b8', marginTop: 60, fontSize: scaleFont(14) },
});
