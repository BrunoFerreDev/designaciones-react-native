...
import { GetDesignacionDTO, GetDesignadosDTO, EstadoDesignacion } from '../../types';
...
const ESTADO_LABEL: Record<number, string> = {
  0: 'Pendiente', 1: 'Confirmada', 2: 'Finalizada', 3: 'Cancelada', 4: 'Suspendida',
};

const ESTADO_COLOR: Record<number, string> = {
  0: '#f39c12', 1: '#27ae60', 2: '#3498db', 3: '#e74c3c', 4: '#9b59b6',
};
...
export default function DesignacionDetalle() {
        ...
        { text: 'Cancelar', style: 'cancel' },
        {
          ...
          onPress: async () => {
            try {
              ...
              const msg = e?.response?.data?.message || e?.message || 'Error al desasignar';
    ...
    Alert.alert(
      ...
      [
        { text: 'Cancelar', style: 'cancel' },
        {
          ...
          onPress: async () => {
            try {
              const updated = await designacionService.reprogramarDesignacion(designacion.idDesignacion);
              setDesignacion(updated);
              Alert.alert('Éxito', 'La designación fue reprogramada exitosamente.');
              loadData();
            } catch (e: any) {
    ...
    Alert.alert(
      ...
      [
        { text: 'Cancelar', style: 'cancel' },
        {
          ...
          onPress: async () => {
            try {
              await designacionService.eliminarDesignacion(designacion.idDesignacion);
              Alert.alert('Eliminada', 'La designación ha sido eliminada con éxito', [
                { text: 'OK', onPress: () => navigation.goBack() },
              ]);
            } catch (e: any) {
              const msg = e?.response?.data?.message || e?.message || 'Error al eliminar designación';
              Alert.alert('Error', msg);
            }
          },
        },
  ...
  if (!designacion) return <Text style={styles.empty}>Designación no encontrada</Text>;
...
const styles = StyleSheet.create({
  ...
  cardHeader: {
    ...
    backgroundColor: '#fff', padding: scaleFont(14), borderRadius: 14,
    borderWidth: 1, borderColor: '#e2e8f0', elevation: 1, marginBottom: 12,
  },
  ...
  emptyArbitrosCard: {
    backgroundColor: '#fff', borderRadius: 10, padding: scaleFont(16),
    alignItems: 'center', borderWidth: 1, borderColor: '#e2e8f0', marginBottom: 16,
  },
  emptyText: { color: '#94a3b8', fontSize: scaleFont(13) },
  arbitroCard: {
    flexDirection: 'row', alignItems: 'center',
    backgroundColor: '#fff', borderRadius: 10, padding: scaleFont(12),
    marginBottom: 8, elevation: 1, borderWidth: 1, borderColor: '#e2e8f0',
  },
  arbitroNombre: { fontSize: scaleFont(14), fontWeight: '700', color: '#0f172a' },
  arbitroInfo: { fontSize: scaleFont(11), color: '#64748b', marginTop: 2 },
  arbitroMonto: { fontSize: scaleFont(12), color: '#16a34a', fontWeight: '700', marginTop: 3 },
