...
import {
  ...
  ActivityIndicator, Alert, TextInput, Switch, Modal, RefreshControl,
} from 'react-native';
...
export default function PerfilScreen() {
  ...
  const [estadoCuenta, setEstadoCuenta] = useState<EstadoCuentaArbitroDTO | null>(null);
  ...
  async function handleToggleDisp(dia: 'sabado' | 'domingo', valor: boolean) {
    ...
    try {
      ...
      const msg = e?.response?.data?.message || e.message || 'Error al guardar disponibilidad';
        ...
        <>
          ...
          <View style={styles.userCard}>
            ...
            <View style={{ flex: 1 }}>
              ...
              <Text style={styles.userPhone}>📱 {perfil?.whatsapp || authArbitro?.username}</Text>
          ...
          <View style={styles.card}>
            ...
            <View style={styles.rowDetail}>
              ...
              <Text style={styles.detailValue}>{perfil?.talleCamiseta || 'No definido'}</Text>
            </View>
            <View style={styles.rowDetail}>
              <Text style={styles.detailLabel}>Talle Short:</Text>
              <Text style={styles.detailValue}>{perfil?.talleShort || 'No definido'}</Text>
            </View>
            <View style={styles.rowDetail}>
              <Text style={styles.detailLabel}>Dispone de Auto / Movilidad:</Text>
              <Text style={styles.detailValue}>{perfil?.tieneAuto ? '✅ Sí' : '❌ No'}</Text>
            </View>
          </View>
          ...
          <View style={styles.card}>
            <View style={styles.cardHeaderRow}>
              <Text style={styles.cardTitle}>Mi Disponibilidad Fin de Semana</Text>
          ...
          {estadoCuenta && (
            <View style={styles.card}>
              ...
              <View style={styles.rowDetail}>
                ...
                <Text style={[styles.detailValue, { color: estadoCuenta.totalDeudaPrestamos > 0 ? '#e74c3c' : '#27ae60' }]}>
                  ${estadoCuenta.totalDeudaPrestamos}
                </Text>
              </View>
              <View style={styles.rowDetail}>
                <Text style={styles.detailLabel}>Préstamos Activos:</Text>
                <Text style={styles.detailValue}>{estadoCuenta.prestamosActivosCount}</Text>
              </View>
              <View style={styles.rowDetail}>
                <Text style={styles.detailLabel}>Gastos por Recuperar:</Text>
                <Text style={styles.detailValue}>${estadoCuenta.totalGastosConRecuperoPendientes}</Text>
              </View>
            </View>
          )}
          ...
          <View style={styles.card}>
            ...
            >
              <Text style={styles.btnPasswordText}>🔒 Cambiar Contraseña</Text>
            </TouchableOpacity>
          </View>
        </>
      )}
      ...
      <Modal visible={modalPassVisible} transparent animationType="slide">
        <View style={styles.modalOverlay}>
          <View style={styles.modalBox}>
            ...
            <Text style={styles.label}>Nueva Contraseña (mínimo 6 chars) *</Text>
            <TextInput
              style={styles.input}
              value={passNueva}
...
const styles = StyleSheet.create({
  ...
  content: { padding: scaleFont(16), paddingTop: scaleFont(38), paddingBottom: scaleFont(44) },
  header: {
    flexDirection: 'row', alignItems: 'center',
    justifyContent: 'space-between', marginBottom: scaleFont(16),
  },
  btnBack: { paddingVertical: scaleFont(6), paddingHorizontal: scaleFont(10), borderRadius: 8, backgroundColor: '#e2e8f0' },
  btnBackText: { fontSize: scaleFont(12), fontWeight: '700', color: '#334155' },
  headerTitle: { fontSize: scaleFont(18), fontWeight: 'bold', color: '#0f172a' },
  btnLogout: {
    paddingVertical: scaleFont(6), paddingHorizontal: scaleFont(12), borderRadius: 8,
    backgroundColor: '#fff', borderWidth: 1, borderColor: '#e2e8f0',
  },
  btnLogoutText: { fontSize: scaleFont(12), color: '#ef4444', fontWeight: '700' },
  userCard: {
    backgroundColor: '#fff', borderRadius: 14, padding: scaleFont(16),
    flexDirection: 'row', alignItems: 'center', marginBottom: scaleFont(14),
    borderWidth: 1, borderColor: '#e2e8f0', elevation: 1,
  },
  avatar: {
    width: scaleFont(52), height: scaleFont(52), borderRadius: scaleFont(26), backgroundColor: '#1a1a2e',
    alignItems: 'center', justifyContent: 'center', marginRight: scaleFont(14),
  },
  ...
  userCategory: { fontSize: scaleFont(12), fontWeight: '600', color: '#3b82f6', marginTop: 4 },
  card: {
    backgroundColor: '#fff', borderRadius: 12, padding: scaleFont(16),
    marginBottom: scaleFont(12), borderWidth: 1, borderColor: '#e2e8f0', elevation: 1,
  },
  cardHeaderRow: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', marginBottom: 10 },
  cardTitle: { fontSize: scaleFont(14), fontWeight: '700', color: '#0f172a', marginBottom: 10 },
  rolesRow: { flexDirection: 'row', flexWrap: 'wrap', gap: 6 },
  roleChip: { backgroundColor: '#e2e8f0', borderRadius: 6, paddingHorizontal: scaleFont(10), paddingVertical: 4 },
  roleText: { fontSize: scaleFont(11), fontWeight: '700', color: '#334155' },
  rowDetail: {
    flexDirection: 'row', justifyContent: 'space-between',
    paddingVertical: scaleFont(6), borderBottomWidth: 1, borderBottomColor: '#f8fafc',
  },
  detailLabel: { fontSize: scaleFont(12), color: '#64748b' },
  detailValue: { fontSize: scaleFont(12), fontWeight: '700', color: '#0f172a' },
  switchRow: {
    flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center',
    paddingVertical: scaleFont(10), borderBottomWidth: 1, borderBottomColor: '#f1f5f9',
  },
  ...
  btnPassword: {
    ...
    borderRadius: 8, paddingVertical: scaleFont(12), alignItems: 'center', marginTop: 4,
  },
  btnPasswordText: { fontSize: scaleFont(13), fontWeight: '700', color: '#0f172a' },
  modalOverlay: {
    flex: 1, backgroundColor: 'rgba(0,0,0,0.5)',
    justifyContent: 'center', alignItems: 'center', padding: 20,
  },
  modalBox: {
    backgroundColor: '#fff', borderRadius: 14, padding: scaleFont(20),
    width: '100%', maxWidth: 400, elevation: 5,
  },
  modalTitle: { fontSize: scaleFont(18), fontWeight: 'bold', color: '#0f172a', marginBottom: 14 },
  label: { fontSize: scaleFont(12), fontWeight: '700', color: '#475569', marginBottom: 4, marginTop: 10 },
  input: {
    backgroundColor: '#f8fafc', borderWidth: 1, borderColor: '#cbd5e1',
    borderRadius: 8, paddingHorizontal: scaleFont(12), paddingVertical: scaleFont(10), fontSize: scaleFont(14), color: '#0f172a',
  },
  modalActions: { flexDirection: 'row', justifyContent: 'flex-end', gap: 10, marginTop: 18 },
  modalBtn: { paddingVertical: scaleFont(10), paddingHorizontal: scaleFont(16), borderRadius: 8, minWidth: 90, alignItems: 'center' },
